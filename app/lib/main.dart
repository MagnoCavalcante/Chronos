...
class _ConnectionTestScreenState extends State<ConnectionTestScreen> {
  ...
  void _checkStatus() {
    // Se o initState do main foi chamado e este widget foi renderizado, o Flutter iniciou com sucesso.
  ...
  Future<void> _performNetworkCheck() async {
    ...
    try {
      ...
      // Se a tabela não existir, ela lançará um PostgrestException com código 404,
      // o que ainda prova que a conexão com o servidor foi realizada com sucesso e fomos autenticados!
      ...
      setState(() {
        _isNetworkChecking = false;
        // Erros 401 (Unauthorized) ou 403 (Forbidden) indicam falha de autenticação (anon key inválida)
        ...
        if (status == 401 || status == 403 || e.message.toLowerCase().contains('jwt') || e.message.toLowerCase().contains('invalid api key')) {
          ...
          // Qualquer outro código de erro (como 404 table not found, 400 bad request, ou erro de RLS)
          // prova que a conexão e autenticação foram bem-sucedidas e aceitas pelo servidor!
          _hasNetworkSuccess = true;
